...
% gets indices for periods of neurotar movement and angular velocity in intertrial intervals (ITI), in fp rate and time 
...
posSpeedIdx = find(speedC >= thresh); % mid threshold
posSpeedIdx = posSpeedIdx(posSpeedIdx<=eventtimestamps(end));
posSpeedIdx = posSpeedIdx(posSpeedIdx>=151);

susThresh = 30; % sustained threshold 

% determine ITI periods with acceptable movement
for xx = 1:length(posSpeedIdx)
    
  
        if mean(speedC(posSpeedIdx(xx):posSpeedIdx(xx)+400)) <= susThresh ...
                && mean(speedC(posSpeedIdx(xx)-150:posSpeedIdx(xx))) > 10 % filters out quick bouts and bouts not recently initiated
            
            speedIdx(xx) = posSpeedIdx(xx);
