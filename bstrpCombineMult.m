...
% for movement
MovallspeedTrials = trialSpeed((trialIndexSingle ==1), :);
...
pre = 5;
post = 25;
ts = linspace(-pre, post, size(ITIspeed{1,1}{2,1} ,2));

% movement
figure;
plot(1:size(MovallspeedTrials, 2), mean(MovallspeedTrials), 'Color', red)
hold on;
for idx = 1:3
    ITIspeedSingle = [];
    for c = 1:8
        if ~isempty(ITIspeed{1, c})
            ITIspeedSingle = [ITIspeedSingle; ITIspeed{1, c}{idx}];
        end 
    end 
plot(1:size(ITIspeedSingle, 2), mean(ITIspeedSingle), 'Color', greyIdx{idx})
hold on
jbfill(1:size(ITIspeedSingle, 2), (mean(ITIspeedSingle)-(std(ITIspeedSingle, 0, 1)./sqrt(size(ITIspeedSingle, 1)))), ...
    (std(ITIspeedSingle, 0, 1)./sqrt(size(ITIspeedSingle, 1))+mean(ITIspeedSingle)), greyIdx{idx}, greyIdx{idx})
hold on
end 
jbfill(1:size(ITIspeed, 2), (mean(MovallspeedTrials)-(std(MovallspeedTrials, 0, 1)./sqrt(size(MovallspeedTrials, 1)))),...
    (std(MovallspeedTrials, 0, 1)./sqrt(size(MovallspeedTrials, 1))+mean(MovallspeedTrials)), red, red)
hold on
legend off
box off
set(gca, 'color', 'none')
xline(150, '--k', 'TTL or Threshold')
...
[perm.nothTr, ~] = permTest_array(nothing, movTrial, 1000);
% [perm.nothITI, ~] = permTest_array(nothing, ITIsignal, 1000);
...
for idx = 1:3
    ITIsignalSingle = [];
    for c = 1:8
        if ~isempty(ITIsignal{1, c})
            ITIsignalSingle = [ITIsignalSingle; ITIsignal{1, c}{idx}];
        end 
    end 
plot(ts, mean(ITIsignalSingle), 'Color', greyIdx{idx})
hold on
jbfill(ts, (mean(ITIsignalSingle)-(std(ITIsignalSingle, 0, 1)./sqrt(size(ITIsignalSingle, 1)))), ...
    (std(ITIsignalSingle, 0, 1)./sqrt(size(ITIsignalSingle, 1))+mean(ITIsignalSingle)), greyIdx{idx}, greyIdx{idx})
hold on
end
hold on
...
% tmp = find(btsrp.nothing(2,:)<0);
% id = tmp(consec_idx(tmp, thres));
% plot(ts(id), ylinemax*ones(size(ts(id),2), 2)+2, 's', 'MarkerSize', 7, 'MarkerFaceColor',gris,'Color', gris)
% clear tmp id
% tmp = find(btsrp.nothing(1,:)>0);
% id = tmp(consec_idx(tmp, thres));
% plot(ts(id), ylinemax*ones(size(ts(id),2), 2)+2, 's', 'MarkerSize', 7, 'MarkerFaceColor',gris,'Color', gris)
% clear tmp id
% 
...
% id = tmp(consec_idx(tmp, thres));
% plot(ts(id), ylinemax*ones(size(ts(id),2), 2)+2.3, 's', 'MarkerSize', 7, 'MarkerFaceColor',dy,'Color', dy)
% clear tmp id
...
% tmp = find(perm.nothITI(1,:)<0.0167);
% id = tmp(consec_idx(tmp, thres));
% plot(ts(id), (ylinemax-0.3)*ones(size(ts(id),2), 2)+1.8, 's', 'MarkerSize', 5, 'MarkerFaceColor',yel,'Color', yel)
% clear tmp id 
% 
% tmp = find(perm.nothTr(1,:)<0.0167);
% id = tmp(consec_idx(tmp, thres));
% plot(ts(id), (ylinemax-0.3)*ones(size(ts(id),2), 2)+1.7, 's', 'MarkerSize', 5, 'MarkerFaceColor',gree,'Color', gree)
% clear tmp id 
% 
