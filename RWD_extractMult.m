...
[ITIidx, speedITI, speedTrials, ttl] = nt_ITI_movement(ntFile, ttlFile, eventtimestamps, sr); 
...
for tt = 1:3 % 3 is number of thresholds
    
    for m = 1:length(ITIidx{tt,1})
        % Extract the ITI movement window and store it
        if ~isempty(ITIidx{tt,1})
            tracesITI{tt,1}(end + 1, :) = dFoF(ITIidx{tt,1}(m)-before:ITIidx{tt,1}(m)+after-1); % traces of fitted data
        else
            tracesITI{tt,1} = NaN;
        end
        %     tracesGraw{end+1} = lp_normDatG(startIdx(m):endIdx(m)); % traces of filtered green signal
...
tracesITI = cellfun(@rmmissing, tracesITI, 'UniformOutput', false);

%colates
traceData = cell2mat(traces)';
% tracesITI = cell2mat(tracesITI)'; %% ended work here %%
...
for tt = 1:3
    ...
    if ~isempty(tracesITI{tt,1})
        traceDataSDITI = std(tracesITI{tt,1}(:,1:pre*sr),0,2);
        ZdFoFITI{tt,1} = (tracesITI{tt,1} - mean(tracesITI{tt,1}(:,1:pre*sr),2))./traceDataSDITI;
    else
        ZdFoFITI{tt,1} = NaN;
    end
...
ZdFoFITI = cellfun(@rmmissing, ZdFoFITI, 'UniformOutput', false);
...
if ch ==1
    ...
    sesdat.speedITI = speedITI; % speed iti is cell
